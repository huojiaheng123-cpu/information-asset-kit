...
这个 skill 不只是播放检查底座，还要默认提供高质量视频理解：看清画面、听懂口播、拆解观点、判断可信度，并把对使用者、团队或目标受众有用的东西沉淀成可行动的结论。

`douyin-video-analyst` 的短视频学习资产沉淀能力已合并到本 skill。抖音、TikTok、视频号、小红书视频等短视频链接，都按本 skill 的网页视频/短视频分支处理。
...
- 视频分析不能停留在“讲了什么”，还要回答“这是什么意思、靠不靠谱、对使用者/团队有什么启发、下一步该做什么”。
- 遇到 AI 工具、AI 项目、能力变化、行业趋势、工作流或未来机会类视频时，必须额外做“价值迁移判断”：这个能力成熟后什么会变便宜、什么会更值钱、现有工作方式会怎样改变、可以放大什么、应该减少依赖什么、人还能贡献什么。
- 如果用户想把短视频沉淀成学习资产，输出要包含：核心观点、可信度、对使用者/团队的直接启发、今天/本周/长期行动，以及可转化成内容、脚本、产品、服务、skill 或项目的资产。
- 如果输入其实是公众号文章、网页文章、文档或纯文本，不要声称“看过视频”；应按文本/资料分析处理，并明确证据来自文字而不是视频播放。
...
4. **做高级视频理解**
   ...
   - 提炼启发：对使用者/团队当前业务、技能、内容生产、AI 工具链或资产沉淀有什么意义；如果没有上下文，则从一般创作者、运营者、学习者或团队视角判断。
   - 对 AI 工具、AI 项目、能力变化、行业趋势、工作流或未来机会类视频，补充价值迁移判断：不要只追工具，要判断 AI 让哪种能力变便宜，以及人的价值会迁移到哪里。
...
5. **交付结果**
   ...
   - 如果要归档成知识资产，本 skill 自身必须完成视频拆解、可信度判断、启发提炼、行动建议和可转化资产整理。
...
以前 `douyin-video-analyst` 负责抖音/TikTok 短视频学习资产沉淀。现在该职责并入本 skill：

- 平台不再是主分类，视频理解才是主能力。
- 抖音/TikTok/视频号/小红书视频都是网页短视频输入。
- 短视频归档不再需要单独调用 `douyin-video-analyst`。
- 只有用户明确要求维护旧抖音 skill 时，才查看 `douyin-video-analyst-skill` 目录。
