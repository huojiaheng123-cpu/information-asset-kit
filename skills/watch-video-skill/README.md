...
`douyin-video-analyst` 的能力已经合并进 `watch-video-skill`。

现在按能力分，不按平台分：

- `watch-video-skill`：唯一主入口，负责视频观看、证据、转写、观点拆解、可信度、启发、行动、学习资产沉淀。
- `douyin-video-analyst`：保留为 legacy 兼容说明，不再作为新任务主入口。
- 抖音、TikTok、视频号、小红书视频都是 `watch-video-skill` 的短视频分支。

如果输入是公众号文章、网页文章或纯文本，不要走视频播放取证；应按文本/资料分析处理，并说明证据来自文字。
